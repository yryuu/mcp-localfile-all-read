...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'search_content': {
                const path = resolvePath(args.path);
                const results = await searchFiles(path, args.pattern, {
                    rootPath,
            ...
            case 'find_files': {
                const path = resolvePath(args.path);
                const files = await findFiles(path, args.name_pattern, {
                    rootPath,
                    caseSensitive: args.case_sensitive,
                });
                return {
            ...
            case 'read_excel_sheet': {
                const path = resolvePath(args.path);
                const content = await readExcelSheetAsString(path, args.sheet_name);
                return {
                    content: [
                        {
                            type: 'text',
